...
it.layer(TestLayer, { excludeTestServices: true })("WorkspaceEntries", (it) => {
  ...
  describe("list", () => {
    ...
    it.effect(
      "includeHidden lists dotfiles and gitignored files, still skipping .git and node_modules",
      () =>
        Effect.gen(function* () {
          const cwd = yield* makeTempDir();
          ...
          // .gitignore is a normal file and must still be listed; only the
          // .git directory itself is skipped.
          expect(paths).toContain(".gitignore");
          expect(
            paths.some((entryPath) => entryPath === ".git" || entryPath.startsWith(".git/")),
          ).toBe(false);
